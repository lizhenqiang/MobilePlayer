...
public class MyListAdapter extends BaseAdapter {
    ...
    private ArrayList<AllDataBean.ListBean> mediaItems;


    public MyListAdapter(Context context, ArrayList<AllDataBean.ListBean> mediaItems){
    ...
    public int getItemViewType(int position) {
        int itemViewType = -1;
        //根据位置，从列表中得到一个数据对象
        AllDataBean.ListBean listBean = mediaItems.get(position);
...
//        TextView iv_image_icon;
    ...
    private void bindData(int position, int itemViewType, ViewHolder viewHolder) {
        //根据位置得到数据,绑定数据
        AllDataBean.ListBean mediaItem = mediaItems.get(position);
...
//                    x.image().bind(viewHolder.iv_image_icon, mediaItem.getImage().getBig().get(0));
     ...
     * @param datas
     */
    public void setData(ArrayList<AllDataBean.ListBean> datas) {
